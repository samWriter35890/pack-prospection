...
**Lire les dix champs, même ceux dont ce skill n'a pas l'usage.** C'est délibéré : la lecture sert toute la fenêtre, et `rediger-email`, `accroche-linkedin` ou `creer-opportunite` s'en serviront ensuite sans repayer l'appel. Un skill qui n'en lirait que trois obligerait le suivant à tout relire.
...
`pageSize` 25 sur les deux premiers, 10 sur les deux autres. **Quatre appels pour les listes, pas cinq** : celui du contexte ci-dessus ne compte pas, il est payé une fois pour toute la fenêtre.
