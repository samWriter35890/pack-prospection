...
```
updateRecords  Contacts  id=15  {"Statut relation": "À contacter", "Dernier échange": "2026-08-11"}
```

**Faire avancer, jamais reculer**, et cela vaut pour les deux : un contact déjà `En discussion`, `Client` ou `Dormant` n'y redescend pas, et une date de dernier échange **postérieure** à celle qu'on s'apprête à écrire se garde telle quelle. La règle complète est dans `enregistrer-echange`, étape 3, elle ne se recopie pas plus loin que ceci. Sans elle, un contact à qui on a écrit hier reste rangé avec ceux qu'on n'a jamais approchés, et le champ ne veut plus rien dire.
